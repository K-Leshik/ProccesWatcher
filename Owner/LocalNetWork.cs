...
namespace Owner
{
    class LocalNetWork
    {
        ...
        private static extern NERR NetServerEnum( [MarshalAs(UnmanagedType.LPWStr)]string ServerName,
                                                 int Level, 
                                                 out IntPtr BufPtr,
                                                 int PrefMaxLen, 
                                                 ref int EntriesRead,
            ...
            try
            {
                ...
                if ((err == NERR.NERR_Success || err == NERR.ERROR_MORE_DATA) && pInfo != IntPtr.Zero)
                {
                    ...
                    for (int i = 0; i < etriesread; i++)
                    {
                        ...
                        srvs.Add(si.sv101_name.ToString());
        ...
        public static string GetIPByHostName(string host_name)
        {
            if(host_name == null || host_name == string.Empty)
            {
                return null;
            }

            IPAddress[] ips = Dns.GetHostEntry(host_name).AddressList;
